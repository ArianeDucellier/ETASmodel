...
families = list.files("../data/Ducellier_2022/catalogs", pattern="*.txt")
...
for (i in 28:length(families)){
    
    catalog = families[i]
    output_dir <- paste("models_Ducellier_2022/", catalog, sep="")
    if (!dir.exists(output_dir)){
        dir.create(output_dir)
    }
    
    # Read data
    filename = paste("../data/Ducellier_2022/catalogs/", catalog, ".txt", sep="")
