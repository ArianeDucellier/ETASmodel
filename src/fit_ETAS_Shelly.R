...
families = list.files("../data/Shelly_2017/catalogs", pattern="*.txt")
...
for (i in 1:length(families)){
    ...
    catalog = families[i]
    output_dir <- paste("models_Shelly_2017/", catalog, sep="")
    if (!dir.exists(output_dir)){
        dir.create(output_dir)
    }

    # Read data
    filename = paste("../data/Shelly_2017/catalogs/", catalog, ".txt", sep="")
